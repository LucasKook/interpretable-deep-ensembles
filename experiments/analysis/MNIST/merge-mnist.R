# Merge all val, test CDF of all models, Y
# Andrea Goetschi
# May 2022
...
cdf_cinll <- rbind(cdfval_cinll, cdftest_cinll)
cdf_cirps <- rbind(cdfval_cirps, cdftest_cirps)

write.csv(cdf_cinll, paste0(out_dir, "mnist_merged_cdf_cinll.csv"), row.names = FALSE)
write.csv(cdf_cirps, paste0(out_dir, "mnist_merged_cdf_cirps.csv"), row.names = FALSE)
