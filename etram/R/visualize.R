...
#' @export
plot_metrics <- function(met, indiv_met = NULL, indiv_ens = TRUE,
  ...
  all_metrics <- c("nll", "rps", "qwk", "eqwk", "acc", "eacc",
                   "binnll", "brier", "auc", "eauc", "binacc", "ebinacc",
                   "cint", "cslope")
  ...
  if (indiv_ens) {
    ...
    p <- ggplot(avgs, aes(x = topn, y = val, color = method)) +
      facet_wrap(~ metric, scales = "free", nrow = nrow, ncol = ncol) +
      geom_line() +
      geom_point() +
      labs(x = "Top-K ensemble", y = "Test performance") +
